 ...
 * 
 * @desc Returns public statuses that match one or more filter predicates. Multiple parameters 
 * may be specified which allows most clients to use a single connection to the Streaming API.
 * Both GET and POST requests are supported, but GET requests with too many parameters may 
 * cause the request to be rejected for excessive URL length. Use a POST request to avoid 
 * long URLs. The track, follow, and locations fields should be considered to be combined 
 * with an OR operator. track=foo&follow=1234 returns Tweets matching "foo" OR created by 
 * user 1234. The default access level allows up to 400 track keywords, 5,000 follow userids 
 * and 25 0.1-360 degree location boxes. If you need elevated access to the Streaming API, 
 * you should explore our partner providers of Twitter data at 
 ...
 * @param follow
 * 
 * A comma-separated list of user IDs, indicating the users whose Tweets should be delivered 
 * on the stream. Following protected users is not supported. For each user specified, 
 * the stream will be contain:
 ...
 */
$params['follow'] = 'A comma separated list of user IDs,';
/**
 * @param follow
 * 
...
$statuses = $twitter->post("statuses/filter", $params);
print_r($statuses);
?>